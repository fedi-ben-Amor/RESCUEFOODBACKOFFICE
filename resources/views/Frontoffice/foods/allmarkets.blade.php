    ...
    <div class="row pt-3 pt-sm-0">
        @foreach($restaurants as $restaurant)
            <div class="col-md-4 col-sm-6 mb-grid-gutter">
                <a class="card product-card h-100 border-0 shadow pb-2" href="{{ route('foodmarkets.show', $restaurant->id) }}">
                    <span class="badge badge-end badge-shadow bg-success fs-md fw-medium" data-bs-toggle="tooltip" title="Average meal cost">AMC: $17.<small>00</small></span>
                    <img class="card-img-top" src="{{ asset($restaurant->picture) }}" alt="{{ $restaurant->name }}">
                    <div class="bg-white rounded-3 pt-1 px-2 mx-auto mt-n5" style="width: 175px;">
                        <img class="d-block rounded-3 mx-auto" src="{{ asset($restaurant->logo) }}" width="150" alt="{{ $restaurant->name }}">
                    </div>
                    <div class="card-body text-center pt-3 pb-4">
                        ...
                        <a href="{{ route('foodmarkets.show', $restaurant->id) }}" class="btn btn-primary mt-3">Details</a> <!-- Details button -->
                    </div>
                </a>
            </div>
        @endforeach
